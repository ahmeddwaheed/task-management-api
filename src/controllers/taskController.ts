...
const taskController = {
  getAllTasks: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { tasks, totalTasks } = await taskService.getAllTasks(
        userId,
        skip,
        limit
      );
  ...
  createTask: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const { error, value } = await taskSchema.validate({
        title,
        description,
        status,
        priority,
  ...
  searchTasks: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const tasks = await taskService.searchTasksByParams(userId, {
        priority,
        dueDate,
        status,
        title,
